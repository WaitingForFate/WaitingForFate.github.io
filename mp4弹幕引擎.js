...
class DOM弹幕引擎 {
    ...
    发射_draw(){
        if (this.是否发射){
                    ...
                    // console.log("in draw可发射弹幕数 "+应发射弹幕i.length)
                    for (let i = 0; i < this.航道数量; i++)
                        {
...
//-------------结束
function Mp4_draw_DanMu (Vedio_id,XML_path,debugIndex) {
    //---------初始化
        let DD = new DOM弹幕引擎();
        DD.加载弹幕(XML_path)
        DD.建立航道()
...
//------------下面是容易变化的视频事件处理部分
    ...
    function timeUpdate处理() {
        ...
        //   console.log(DD.videoPlayer.currentTime);
        }
//------------------------------结束
//------------下面是视频事件监听部分
    
    
    DD.videoPlayer  = document.getElementById(Vedio_id);
    DD.videoPlayer.ontimeupdate = timeUpdate处理
    DD.videoPlayer.onplaying = playing处理
    DD.videoPlayer.onpause =pause处理

    // DD.videoPlayer.onened = function () { myonened(); };
    
    

    DD.videoPlayer.addEventListener('ended', function () { console.log("播放结束"); DD.是否发射=0;}, false);
    DD.videoPlayer.addEventListener("loadeddata", function () {console.log("停止加载");DD.是否发射=0;})// DD.ctx.clearRect(0, 0, DD.w, DD.h);DD.是否draw[1]=0;//低级别控制}, false);
//------------------------------结束
//------------下面是键盘/触摸事件处理
    ...
    function 点击事件处理(e){
        ...
        console.log("点击");  
        if (DD.videoPlayer.paused) {DD.videoPlayer.play();
